...
class Racer implements ______________{ // input the interface name
   ...
   _____________{  // input the abstrac function name
      System.out.println("age()=" + Scheduler.age() + ", "
         + name + " is running");
      for (int m = 1; m <= M; m++)
      {
...
class RaceTwoThreads {
   ...
   private final static int numRacers = ____; // fill in a number that how many threads you want to run
   ...
   public static void main(String[] args) {
	  ...
	  _________________________// create a thread function object
      _________________________// create an array of threads
      for (int i = 0; i < numRacers; i++)
         __________________ // create a racer thread 
      for (int i = 0; i < numRacers; i++) {
         __________________ // start each thread
      }
      System.out.println("age()=" + Scheduler.age() + 
      ...
      try {
            for (int i = 0; i < numRacers; i++) 
				______________ // synchronize threads using join()
